...
/*********************************************************************************************************************************************************************************/
      ...
      iarg++;
    } else if ( strcmp(arg[iarg],"Ncons") == 0 ) {
      cons_step_num = atof(arg[iarg+1]);
      iarg++;
/*********************************************************************************************************************************************************************************/
...
/*********************************************************************************************************************************************************************************/  
  // trigger potential energy calculation on next timestep if the conservation term is needed
  
  if (cons_step_num) {
    double cons_etot, ekin;
    cons_etot = compute_etotal() + compute_scalar();
    ekin = temperature->compute_scalar();
    ekin *= 0.5*temperature->dof * force->boltz;
    E = - beta * ( cons_etot - e0 ) / ekin;
...
/*********************************************************************************************************************************************************************************/
        double D = mu * omega[sd] * omega[sd] /
          (velocity_sum * mass[type[i]] * vol ) + E;
/*********************************************************************************************************************************************************************************/
...
/*********************************************************************************************************************************************************************************/  
  // trigger potential energy calculation on the next needed timestep
  
  if ( cons_step_num && (update->ntimestep+1) % cons_step_num == 0 ) {
    pe->addstep(update->ntimestep+1);
  } else if ( cons_step_num && update->ntimestep % cons_step_num == 0 ){
    double cons_etot, ekin;
    cons_etot = compute_etotal() + compute_scalar();
    ekin = temperature->compute_scalar();
    ekin *= 0.5*temperature->dof * force->boltz;
